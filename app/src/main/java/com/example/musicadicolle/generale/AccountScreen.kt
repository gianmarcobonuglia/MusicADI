...
@Composable
fun AccountScreen(navController: NavHostController, navigationViewModel: NavigationViewModel) {
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        ...
        if (user != null) {
            AdaptiveLayout(navController = navController, navView = navigationViewModel, url = user.photoUrl.toString()) {
                ...
                Column(
                    ...
                    Column(
                        ...
                        var isUpdating by remember { mutableStateOf(false) } // Stato per mostrare il caricamento

                        TextField(
                            value = newName,
                            onValueChange = { newName = it }, // Rimuoviamo la restrizione di stringa non vuota
                            label = { Text("Cambia nome") }
                        )

                        Spacer(modifier = Modifier.height(8.dp))

                        Button(onClick = { selectPhotoContract.launch("image/*") }) {
                                    ...
                                    user.updateProfile(profileUpdates)
                                        .addOnCompleteListener { task ->
                                            ...
                                            if (task.isSuccessful) {
                                                ...
                                                Log.e("Firebase", "Errore aggiornamento profilo", task.exception)
                                            }
                                        }
                                },
                                enabled = !isUpdating // Disabilita il bottone durante l'aggiornamento
                            ) {
                                if (isUpdating) {
